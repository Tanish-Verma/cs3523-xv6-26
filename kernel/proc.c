...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (queue_level = 0; queue_level < 4; queue_level++)
    {
      int found = 0;
      for (int i = 0; i < NPROC; i++)
      {
        int base = (queue_level == prev_queue_level) ? (prev_index + 1) % NPROC : 0;
        int start = (base + i) % NPROC;
        p = &proc[start];
        acquire(&p->lock);
        if (p->queue_level == queue_level && p->state == RUNNABLE)
        {
          p->state = RUNNING;
          c->proc = p;
          p->sched_count++;
...
void sleep(void *chan, struct spinlock *lk)
{
  ...
  // Reset del_s and del_t for next scheduling slice
  // Check if process was interactive before sleeping
  if (p->del_s >= p->del_t && p->queue_level < NQUEUE - 1)
  {
    p->to_demote = 0;
  }
  p->del_s = 0;
